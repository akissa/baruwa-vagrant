...
if egrep -xq '^# then you will need to either|127\.0\.0\.1 guest$' /etc/hosts; then
cat << 'EOF' > /etc/hosts
...
if egrep -xq '^HOSTNAME=vultr.guest|HOSTNAME=digitalocean$' /etc/sysconfig/network; then
